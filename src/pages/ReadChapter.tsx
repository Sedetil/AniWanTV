...
const ReadChapter = () => {
  ...
  // Add hotkey for toggling full-screen mode
  useHotkeys("f", () => {
  ...
  const openLoFiPlaylist = () => {
    window.open("https://open.spotify.com/playlist/0vvXsWCC9xrXsKd4FyS8kM?si=2c7b79e9d7e94e4b", "_blank");
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className={`flex-1 ${!isFullScreen ? 'pt-16 sm:pt-20 md:pt-24' : 'pt-0'} relative`}>
        {/* Settings Modal */}
        {showSettings && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 sm:p-6" onClick={() => setShowSettings(false)}>
            <div className="bg-background rounded-lg shadow-lg p-4 sm:p-6 max-w-sm sm:max-w-md w-full" onClick={e => e.stopPropagation()}>
        ...
        <div className={`container mx-auto px-2 sm:px-4 md:px-6 ${isFullScreen ? 'max-w-full px-0' : 'py-4 sm:py-6 md:py-8'}`}>
          ...
          {!isFullScreen && (
            <div 
              className={`flex flex-wrap justify-between items-center mb-4 sm:mb-6 sticky top-0 bg-background/95 backdrop-blur-sm z-30 py-2 sm:py-3 gap-2 px-2 sm:px-4 rounded-lg shadow-sm transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
            >
              <div className="flex items-center gap-2 min-w-0 flex-shrink-0">
                <h1 className="text-lg sm:text-xl md:text-2xl font-bold truncate max-w-[120px] sm:max-w-[200px] md:max-w-[300px]">{data.title}</h1>
              </div>
          ...
          {!isFullScreen && (
            <div 
              className={`fixed bottom-2 sm:bottom-4 left-2 right-2 sm:left-4 sm:right-4 bg-background/95 backdrop-blur-sm z-50 py-2 sm:py-3 px-3 sm:px-4 rounded-lg shadow-lg transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
            >
              <div className="flex justify-between items-center max-w-4xl mx-auto">
                ...
                <div className="flex gap-1 sm:gap-2">
                  ...
                  {data.navigation?.chapter_list && (
                    <Link to={`/comic/${getSlug(data.navigation.chapter_list)}`}>
                      <Button variant="secondary" size="icon" className="h-8 w-8 sm:h-9 sm:w-9" title="Chapter List">
